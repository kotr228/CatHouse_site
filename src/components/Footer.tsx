...
export default function Footer({ locale }: FooterProps) {
  ...
  const {
    logoText, logoImageUrl, siteName,
    contactEmail, contactEmailVisible,
    contactTelegram, contactTelegramVisible,
    contactViber, contactViberVisible,
    contactWhatsapp, contactWhatsappVisible,
  } = useSiteSettings();
  ...
  const connectLinks = [
    ...
    contactTelegramVisible && contactTelegram && {
      label: 'Telegram',
      href: contactTelegram.startsWith('http') ? contactTelegram : `https://t.me/${contactTelegram.replace(/^@/, '')}`,
      title: contactTelegram.startsWith('@') ? contactTelegram : `@${contactTelegram.replace(/.*t\.me\//, '')}`,
      icon: (
    ...
    contactViberVisible && contactViber && {
      label: 'Viber',
      href: `viber://chat?number=%2B${contactViber.replace(/\D/g, '')}`,
      title: contactViber,
      icon: (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
    ...
    contactWhatsappVisible && contactWhatsapp && {
      label: 'WhatsApp',
  ...
  ].filter((x): x is ConnectLink => Boolean(x));
